...
use crate::exchanges::bybit::api::types::BybitInstrumentsResponse;
...
pub fn parse_bybit_markets(
    ...
    #[derive(serde::Deserialize)]
    struct BybitResult { list: Vec<BybitListItem> }
    #[derive(serde::Deserialize)]
    struct BybitInstrumentsResponse { result: BybitResult }

    let response: BybitInstrumentsResponse =
        serde_json::from_slice(raw_data).context("Failed to parse Bybit instruments response")?;

    let market_type_db = if market_category.eq_ignore_ascii_case("PERP") { "perps" } else { "spot" };
...
pub fn parse_bybit_klines(raw_data: &[u8], market_symbol: &str) -> Result<Vec<NormalizedKline>> {
    let response: BybitKlineResponse = serde_json::from_slice(raw_data)
        .context("Failed to parse Bybit klines")?;
    
    response.result.list.into_iter().map(|kline| {
        Ok(NormalizedKline {
...
pub fn parse_bybit_trades(raw_data: &[u8], market_symbol: &str) -> Result<Vec<NormalizedTrade>> {
    let response: BybitTradeResponse = serde_json::from_slice(raw_data)
        .context("Failed to parse Bybit trades")?;
        
    response.result.list.into_iter().map(|trade| {
