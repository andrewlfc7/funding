...
pub struct VolLiquidityResponse {
    ...
    /// NEW: per-symbol volume stats for frontend (use in place of mcap comparisons)
...
/// NEW: weekly & average USD volumes + ratios (no mcap needed)
#[derive(Debug, Serialize)]
...
pub async fn get_vol_liquidity(
    ...
    if let Some(n) = q.topN {
        ...
        for (sym, mid, _) in top {
            ...
            // rolling vol z of log-returns
            let lr = log_returns(&close);
            let win_fast = tf.steps_per_day().max(6);
            let (_, s) = rolling_mean_std(&lr, win_fast);
            let win_slow = (5 * win_fast).max(6).min(n.max(6));
            let (m_slow, sd_slow) = rolling_mean_std(&s, win_slow);
            let volz: Vec<f64> = s.iter().enumerate().map(|(i, &v)| {
                let sd = sd_slow[i];
                if sd.is_finite() && sd > 0.0 && m_slow[i].is_finite() { (v - m_slow[i]) / sd } else { f64::NAN }
            }).collect();

            // EWMA USD volume z-score (for distribution)
            let vol_ewma = super::ewma_span(&usd, win_fast.max(24));
            let vol_ewma_z = zscore_series(&vol_ewma, win_slow);
            ...
            for i in start..n {
                ...
                // 24h range over last TF-day worth of bars
            ...
            // ===== NEW: weekly & average USD volume + ratios (use last matured index) =====
            let end = n - 1;
            let steps_day = tf.steps_per_day();
    ...
    // NEW: single-coin summary (use last bar)
    let end = n - 1;
    let steps_day = tf.steps_per_day();
    let steps_week = steps_day * 7;
    let start_week = end.saturating_sub(steps_week - 1).max(start);
