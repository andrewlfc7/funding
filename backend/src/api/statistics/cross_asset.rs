...
async fn compute_cross_asset_analytics(
    ...
    if let Some(n) = q.topN {
        if let Ok(top) =
            top_markets_by_usd_volume_live(&pool, &q.exchange, &q.marketType, days as i32, n).await
        {
    ...
    for sym in &all_symbols {
        if let Ok(mid) =
            super::resolve_market_id_with_data(&pool, &q.exchange, sym, &q.marketType, since_unix)
                .await
        {
            mids.push(mid);
    ...
    for data in resampled_data.values().skip(1) {
        ...
        while i < common_timestamps.len() && j < data.1.len() {
            if common_timestamps[i] == data.1[j] {
                intersection.push(common_timestamps[i]);
                i += 1;
                j += 1;
            } else if common_timestamps[i] < data.1[j] {
                i += 1;
            } else {
                j += 1;
            }
    ...
    let final_betas: Vec<f64> = returns
        .iter()
    ...
    CrossAssetAnalyticsResponse {
        ...
        covariance_matrix: CovMatrix {
            coins: coins.clone(),
            matrix: cov_matrix_values,
            timestamp: last_ts,
        },
        index: index_sym,
        rolling_corr,
        rolling_beta,
        coin_betas: CoinBetas {
            coins,
            betas: final_betas,
        },
    }
