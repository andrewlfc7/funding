    ...
    D: Deserializer<'de>,
{
    ...
    impl<'de> Visitor<'de> for StrOrSeq {
        ...
        fn expecting(&self, f: &mut fmt::Formatter) -> fmt::Result {
            write!(f, "string, CSV, or sequence")
        }
...
fn is_stable_symbol(sym: &str) -> bool {
    ...
    const STABLES: &[&str] = &[
        "USD", "USDT", "USDC", "FDUSD", "FUSD", "TUSD", "BUSD", "DAI", "PYUSD", "USDE", "USDD",
        "USDP", "GUSD", "USDJ", "USDX", "FRAX", "LUSD", "SUSD", "MIM", "DOLA", "EURS", "EURC",
        "EURT", "CRVUSD", "USDL",
    ];
    if STABLES.contains(&s.as_str()) {
        return true;
    }
...
fn dow_of(ts: i64) -> Option<u8> {
    ...
    time::OffsetDateTime::from_unix_timestamp(ts)
        .ok()
        .map(|dt| dt.weekday().number_from_monday() as u8 - 1)
}
fn month_of(ts: i64) -> Option<u8> {
    time::OffsetDateTime::from_unix_timestamp(ts)
        .ok()
        .map(|dt| dt.month() as u8 - 1) // 0..11
}
...
async fn compute_market_seasonality(
    ...
    let mut symset: BTreeSet<String> = q
        .coins
        .clone()
        .unwrap_or_default()
        .into_iter()
    ...
    for (sym, mid) in &sym_mid {
        ...
        for i in 0..ser.len() {
            ...
            if let Some(mo) = month_of(ts) {
                mret[mo as usize] += ret_pct;
                mret_c[mo as usize] += 1;
                mvol[mo as usize] += range_pct;
                mvol_c[mo as usize] += 1;
            }
        ...
        let last_range = if last.low > 0.0 {
            (last.high - last.low) / last.low * 100.0
        } else {
            0.0
        };
    ...
    for i in 0..weekday_vol.len() {
        ...
        for d in 0..7 {
            v1[d] = if weekday_vol_cnt[i][d] > 0 {
                weekday_vol[i][d] / weekday_vol_cnt[i][d] as f64
            } else {
                0.0
            };
            v2[d] = if weekday_vol_usd_cnt[i][d] > 0 {
                weekday_vol_usd[i][d] / weekday_vol_usd_cnt[i][d] as f64
            } else {
                0.0
            };
            v3[d] = if weekday_ret_cnt[i][d] > 0 {
                weekday_ret[i][d] / weekday_ret_cnt[i][d] as f64
            } else {
                0.0
            };
        }
    ...
    for (sym, arr) in monthly_ret {
        ...
        for m in 0..12 {
            ...
            volv[m] = if volcnt[m] > 0 {
                vol[m] / volcnt[m] as f64
            } else {
                0.0
            };
        }
        monthly_seasonality.insert(
            sym,
            MonthlySeasonality {
                returns: retv,
                volatility: volv,
    ...
    for sym in &syms_with_data {
        if let Some(acf) = vol_acf.get(sym) {
            ...
            let pattern = if l1 > 0.5 && l24 > 0.2 {
                "Persistent"
            } else if l1 < 0.0 && l4 < 0.0 {
                "Mean-reverting"
            } else {
                "Neutral"
            };
    ...
    if !intraday_heatmap.is_empty() {
        ...
        for (i, row) in intraday_heatmap.iter().enumerate() {
            ...
            for h in 0..24 {
                ...
                if row[h] > thr {
                    above[h] += 1;
                    assets_above[h].push(syms_with_data[i].clone());
                }
            }
        }
    ...
    for (i, sym) in syms_with_data.iter().enumerate() {
        ...
        weekend_effect.push(WeekendEffect {
            asset: sym.clone(),
            effect: finite(eff),
        });
    }
    ...
    for (zone, win) in zones {
        ...
        timezone_effects.push(TimezoneEffect {
            zone: zone.into(),
            active_hours: win.into(),
            impact: impact.into(),
        });
    }
