...
#[inline]
fn floor_minute(sec: i64, step_min: i64) -> i64 {
...
fn parse_period_days(s: &str) -> i64 {
    ...
    if let Some(p) = s.strip_suffix("d") {
        p.parse::<i64>().unwrap_or(30)
    } else {
        30
    }
}
...
async fn compute_trades_analysis(pool: PgPool, q: TradesAnalysisRequest) -> TradesAnalysisResponse {
    ...
    for (sym, rows) in &by_sym {
        ...
        for r in rows {
            ...
            sum_size += qv;
            count += 1;
            if r.side.eq_ignore_ascii_case("buy") {
                buy += qv;
            } else {
                sell += qv;
            }
        ...
        for r in rows {
            if r.quote >= thr {
                whales.push(WhaleRow {
                    ...
                    side: if r.side.eq_ignore_ascii_case("buy") {
                        "buy".into()
                    } else {
                        "sell".into()
                    },
    ...
    if whales.len() > 100 {
        whales = whales.split_off(whales.len() - 100);
    }
    ...
    for sym in &symbols {
        ...
        let mut row = Vec::with_capacity(grid.len());
        for &g in &grid {
            row.push(*mcnt.get(&g).unwrap_or(&0) as f64);
        }
    ...
    let hist_sym = q
        .histogram_symbol
        .clone()
        .unwrap_or_else(|| symbols.first().cloned().unwrap_or_else(|| "BTC".into()));
    let sizes: Vec<f64> = by_sym
        .get(&hist_sym)
    ...
    let size_dist = if sizes.is_empty() {
        ...
        let mut e = min_e;
        while e <= max_e + 0.0001 {
            bins.push(10f64.powf(e));
            e += 0.25;
        }
        if bins.len() < 2 {
            bins.push(bins[0] * 1.5);
        }
    ...
    let overall_avg = if total_trades > 0 {
        total_usd / total_trades as f64
    } else {
        0.0
    };
    stats.push(StatRow {
        name: "Total USD (period)".into(),
        value: format!("{:.0}", total_usd),
    });
    stats.push(StatRow {
        name: "Total trades (period)".into(),
        value: format!("{}", total_trades),
    });
    stats.push(StatRow {
        name: "Overall avg trade size".into(),
        value: format!("{:.2}", overall_avg),
    });
    let thr = percentile(all_sizes.clone(), q.whale_percentile.clamp(0.9, 0.99));
    stats.push(StatRow {
        name: "Whale threshold (USD)".into(),
        value: format!("{:.0}", thr),
    });
    stats.push(StatRow {
        name: "Assets".into(),
        value: format!("{}", symbols.len()),
    });
