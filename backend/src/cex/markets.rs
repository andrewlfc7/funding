...
fn is_stablecoin_symbol(symbol: &str) -> bool {
    let stablecoins = [
        "USDT", "USDC", "DAI", "BUSD", "TUSD", "FDUSD", "GUSD", "PYUSD", "USDD", "LUSD", "USDP", "FRAX","USD1"
    ];
    stablecoins.contains(&symbol.to_ascii_uppercase().as_str())
}


async fn fetch_markets_for_exchange(exchange_name: &str, market_type: CexMarketType) -> Result<Vec<CexMarket>> {
    match exchange_name {
        "binance" => {
        ...
        "bybit" => {
            ...
            match market_type {
                CexMarketType::Spot => {
                    ...
                    parse_bybit_markets(&raw_spot, exchange_name, "SPOT").context("parse bybit spot")
                }
                CexMarketType::Perps => {
                    let raw_perp = client
                    ...
                    parse_bybit_markets(&raw_perp, exchange_name, "PERP").context("parse bybit perps")
                }
            }
        }
